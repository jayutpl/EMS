...
public:
void modbus_configure(long baud, byte _slaveID,unsigned int parity, unsigned int stop_bit, byte _TxEnablePin, unsigned int _holdingRegsSize, unsigned char _lowLatency);
bool modbus_update(signed int *holdingRegs);
unsigned long last_connected_millis=0;
bool isStore=false;
};
